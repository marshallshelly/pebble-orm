...
func main() {
	...
	if err != nil {
		...
		log.Printf("✅ Inserted product: ID=%d, Name=%s, Price=%.2f",
	...
	if len(insertedProducts) > 0 {
		...
		if err != nil {
			...
			log.Printf("✅ Inserted order: ID=%d, OrderNumber=%s, Total=%.2f",
	...
	// Example 4: SELECT Orders with JOIN
	log.Println("\n--- Example 4: SELECT Orders ---")
	orders, err := builder.Select[models.Order](qb).
		Where(builder.Eq(builder.Col[models.Order]("Status"), "pending")).
	...
	if err != nil {
		...
		for _, o := range orders {
			log.Printf("  - ID=%d: %s (Qty: %d, Total: $%.2f)",
				o.ID, o.OrderNumber, o.Quantity, o.TotalPrice)
		}
	}
	...
	if len(orders) > 0 {
		...
		if err != nil {
			...
			log.Printf("✅ Updated %d order(s) to 'shipped' status", count)
		}
	}

	// Example 6: COUNT with WHERE
	log.Println("\n--- Example 6: COUNT ---")
	...
	if err != nil {
		...
		log.Printf("✅ Total in-stock products: %d", productCount)
	}

	log.Println("\n=== Examples Complete ===")
	log.Println("\n💡 Identity Types Used:")
	log.Println("   • Product.ID: GENERATED ALWAYS AS IDENTITY (strict, recommended)")
	log.Println("   • Order.ID: GENERATED BY DEFAULT AS IDENTITY (flexible)")
}
